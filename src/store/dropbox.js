...
export default {
  ...
  getters: {
    ...
    isImage: () => entry => {
      return ["jpg", "gif", "jpeg", "png"].includes(
        RegExp(/\.(.*?)$/)
          .exec(entry.name)[1]
          .toLowerCase()
      );
  ...
  actions: {
    connectDropbox({ state, rootState }) {
      return new Promise((resolve, reject) => {
        if (rootState.settings.dropboxAccessToken) {
          console.log("Connecting to Dropbox");
          state.dbx = new Dropbox.Dropbox({ accessToken: rootState.settings.dropboxAccessToken, fetch });
          resolve();
        }
        reject("You may want to check settings for your Dropbox credentials");
      });
    },
    ...
    deleteFromDropbox({ state, rootState, commit }, path) {
      if (path) {
        ...
        state.dbx.filesDelete({ path: rootState.markdown.path }).then(() => commit("resetApplication"));
      }
    },
    ...
    filterEntries({ state }, entries) {
      return new Promise(res => {
        if (state.sort === sort.DATE) {
          ...
          res(entries.sort((a, b) => (a.name > b.name ? 1 : a.name < b.name ? -1 : 0)));
        }
      });
    },
