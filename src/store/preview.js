...
export default {
  ...
  getters: {
    ...
    activeOrientation: state =>
      state.sizes.find(size => size.active === true).orientation,
  ...
  actions: {
    updatePreview({ state, rootState }) {
      ...
      let markdown = md({ html: true, linkify: true }).render(
        window.codemirror.getValue()
      );
    ...
    publishMarkdown({ state, rootState, getters }, inkFriendly) {
      ...
      iframe.srcdoc = `
        ...
        getters.activeStyle
      }.css?v=${state.styleVersion}" rel="stylesheet"/>
          </head>
          <body class="pages ${getters.activeSize} ${
        inkFriendly ? "inkFriendly" : "showNoMercy"
      }">
